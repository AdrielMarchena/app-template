...
void Framebuffer::Invalidate()
{
	...
	ColectionTextureSpecification texture_specifications;
	...
	if (m_ColorAttachmentSpecs.size())
	{
		...
		CreateTexture(multisample, m_ColorTextures.data(), m_ColorTextures.size());

		for (size_t i = 0; i < m_ColorTextures.size(); i++)
		{
			BindTexture(multisample, m_ColorTextures[i]);

			switch (m_ColorAttachmentSpecs[i].TextureFormat)
			{
				case FrambufferTextureFormat::RGBA8:
					AttachColorTexture(texture_specifications, m_ColorTextures[i], m_Specs.samples, GL_RGBA8, m_Specs.width, m_Specs.height, i, GL_RGBA);
					break;
				case FrambufferTextureFormat::RGB:
					AttachColorTexture(texture_specifications, m_ColorTextures[i], m_Specs.samples, GL_RGB, m_Specs.width, m_Specs.height, i, GL_RGB);
					break;
				case FrambufferTextureFormat::RED_INTEGER:
					AttachColorTexture(texture_specifications, m_ColorTextures[i], m_Specs.samples, GL_R32I, m_Specs.width, m_Specs.height, i, GL_RED_INTEGER);
					break;
			}
		}
	}
	...
	if (depth_exist)
	{
		CreateDepthBuffers(multisample, &m_DepthBuffer, 1);

		switch (m_DepthAttachmentSpec.TextureFormat)
		{
			case FrambufferTextureFormat::DEPTH24STENCIL8:
				AttachDepthBuffer(m_DepthBuffer, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT, m_Specs.width, m_Specs.height);
				break;
		}
	}
...
void Framebuffer::ClearAttachment(uint32_t index, int value)
{
	...
	BindTexture(false, m_ColorTextures[index]);
